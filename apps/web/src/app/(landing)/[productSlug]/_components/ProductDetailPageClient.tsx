...
export function ProductDetailPageClient({ productSlug: slug }: { productSlug: string }) {
  // Fetch the detailed product data using your custom hook
  const { data: product, isLoading, isError, error } = useProduct.getBySlug(slug);

  // Helper to format category name for breadcrumbs
  const formatCategoryName = (name: string) => name.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
  ...
  if (isLoading) {
    return (
      <MaxWidthWrapper className="py-12">
        ...
        <Skeleton className="h-96 w-full rounded-2xl bg-neutral-800 mt-8" /> {/* Related Products */}
      </MaxWidthWrapper>
    );
  }
    ...
    return (
      <MaxWidthWrapper className="py-20 text-center">
        ...
        <p className="mt-4 text-neutral-300">
          {error.message || "We couldn't find the product you're looking for. Please try another one."}
        </p>
        <Link href="/" className="mt-6 inline-flex items-center text-blue-400 hover:underline">
  ...
  return (
    <div className="min-h-screen bg-neutral-900 dark:bg-gray-950 pb-12">
      <MaxWidthWrapper className="py-8">
        ...
        <div className="mb-6 flex items-center text-sm text-neutral-400">
          ...
          {product.category && (
            <>
              <Link href={`/categories/${product.category.slug}`} className="hover:text-blue-400">
