...
async function main() {
  ...
  console.log('Creating users...');
  const users = await Promise.all([
    prisma.user.create({ data: { id: 'user_1', name: 'Alice Johnson', email: 'alice@example.com', emailVerified: true, createdAt: new Date(), updatedAt: new Date(), } }),
    prisma.user.create({ data: { id: 'user_2', name: 'Bob Williams', email: 'bob@example.com', emailVerified: true, createdAt: new Date(), updatedAt: new Date(), } }),
    prisma.user.create({ data: { id: 'user_3', name: 'Charlie Brown', email: 'charlie@example.com', emailVerified: true, createdAt: new Date(), updatedAt: new Date(), } }),
    prisma.user.create({ data: { id: 'user_4', name: 'Diana Miller', email: 'diana@example.com', emailVerified: true, createdAt: new Date(), updatedAt: new Date(), } }),
    prisma.user.create({ data: { id: 'user_5', name: 'Ethan Davis', email: 'ethan@example.com', emailVerified: true, createdAt: new Date(), updatedAt: new Date(), } }),
  ]);
  ...
  const furniture = await prisma.category.create({
    data: { name: 'Furniture', slug: 'furniture', description: 'Home and office furnishings' },
  });
  const books = await prisma.category.create({
    data: { name: 'Books', slug: 'books', description: 'Printed and digital literature' },
  });

  // Subcategories of Electronics
  const laptops = await prisma.category.create({
    data: { name: 'Laptops', slug: 'laptops', description: 'Portable computing devices', parentId: electronics.id },
  });
  const smartphones = await prisma.category.create({
    data: { name: 'Smartphones', slug: 'smartphones', description: 'Mobile communication devices', parentId: electronics.id },
  });

  // Subcategories of Furniture
  const sofas = await prisma.category.create({
    data: { name: 'Sofas', slug: 'sofas', description: 'Comfortable seating for living rooms', parentId: furniture.id },
  });
  const chairs = await prisma.category.create({
    data: { name: 'Chairs', slug: 'chairs', description: 'Various types of seating chairs', parentId: furniture.id },
  });

  // Sub-subcategory of Sofas
  const sectionalSofas = await prisma.category.create({
    data: { name: 'Sectional Sofas', slug: 'sectional-sofas', description: 'Modular sofas for flexible seating', parentId: sofas.id },
  });

  // Subcategories of Books
  const fiction = await prisma.category.create({
    data: { name: 'Fiction', slug: 'fiction', description: 'Imaginative literary works', parentId: books.id },
  });
  ...
  const products = await Promise.all([
    ...
    prisma.product.create({
      data: {
        name: 'Quantum Laptop X15', slug: 'quantum-laptop-x15', description: 'A high-performance laptop for professionals.',
        price: new Decimal('1299.99'), stock: 50, sku: 'ELEC-LP-001', categoryId: laptops.id,
        images: { create: [{ url: '/images/danny.jpg', altText: 'Quantum Laptop X15', isPrimary: true }] },
      },
    }),
    prisma.product.create({
      data: {
        name: 'Lightweight Ultrabook', slug: 'lightweight-ultrabook', description: 'Slim and powerful ultrabook for on-the-go productivity.',
        price: new Decimal('999.00'), stock: 75, sku: 'ELEC-LP-002', categoryId: laptops.id,
        images: { create: [{ url: '/images/mana.jpg', altText: 'Lightweight Ultrabook', isPrimary: true }] },
      },
    }),
    // Smartphones
    prisma.product.create({
      data: {
        name: 'Photon Smartphone Pro', slug: 'photon-smartphone-pro', description: 'Latest generation smartphone with a stunning camera.',
        price: new Decimal('799.50'), stock: 150, sku: 'ELEC-SP-001', categoryId: smartphones.id,
        images: { create: [{ url: '/images/manaad.jpg', altText: 'Photon Smartphone Pro', isPrimary: true }] },
      },
    }),
    prisma.product.create({
      data: {
        name: 'Compact Android Phone', slug: 'compact-android-phone', description: 'Powerful features in a pocket-friendly design.',
        price: new Decimal('499.00'), stock: 200, sku: 'ELEC-SP-002', categoryId: smartphones.id,
        images: { create: [{ url: '/images/krisjanis.jpg', altText: 'Compact Android Phone', isPrimary: true }] },
      },
    }),
    // Sectional Sofas
    prisma.product.create({
      data: {
        name: 'Modular Cloud Sofa', slug: 'modular-cloud-sofa', description: 'Extremely comfortable and reconfigurable modular sofa.',
        price: new Decimal('2499.00'), stock: 20, sku: 'FURN-SOF-001', categoryId: sectionalSofas.id,
        tags: ['newDeal'], // Example tag
        images: { create: [{ url: '/images/pavlo.jpg', altText: 'Modular Cloud Sofa', isPrimary: true }] },
      },
    }),
    prisma.product.create({
      data: {
        name: 'L-Shape Sectional', slug: 'l-shape-sectional', description: 'Spacious L-shaped sofa for family gatherings.',
        price: new Decimal('1899.00'), stock: 30, sku: 'FURN-SOF-002', categoryId: sectionalSofas.id,
        images: { create: [{ url: '/images/rafael.jpg', altText: 'L-Shape Sectional', isPrimary: true }] },
      },
    }),
    // Chairs
    prisma.product.create({
      data: {
        name: 'Ergonomic Office Chair', slug: 'ergonomic-office-chair', description: 'Designed for long hours of comfortable work.',
        price: new Decimal('349.00'), stock: 80, sku: 'FURN-CHR-001', categoryId: chairs.id,
        tags: ['exclusive'], // Example tag
        images: { create: [{ url: '/images/danny.jpg', altText: 'Ergonomic Office Chair', isPrimary: true }] },
      },
    }),
    prisma.product.create({
      data: {
        name: 'Accent Armchair', slug: 'accent-armchair', description: 'Stylish armchair to complement any living space.',
        price: new Decimal('199.00'), stock: 60, sku: 'FURN-CHR-002', categoryId: chairs.id,
        images: { create: [{ url: '/images/caroline.jpg', altText: 'Accent Armchair', isPrimary: true }] },
      },
    }),
    // Fiction Books
    prisma.product.create({
      data: {
        name: 'The Midnight Library', slug: 'the-midnight-library', description: 'A profound and moving novel about life choices.',
        price: new Decimal('14.99'), stock: 200, sku: 'BOOK-FIC-001', categoryId: fiction.id,
        images: { create: [{ url: '/images/danny.jpg', altText: 'The Midnight Library book cover', isPrimary: true }] },
      },
    }),
    prisma.product.create({
      data: {
        name: 'Project Hail Mary', slug: 'project-hail-mary', description: 'An enthralling sci-fi adventure from Andy Weir.',
        price: new Decimal('16.50'), stock: 180, sku: 'BOOK-FIC-002', categoryId: fiction.id,
        images: { create: [{ url: '/images/danny.jpg', altText: 'Project Hail Mary book cover', isPrimary: true }] },
      },
    }),
  ]);
  ...
  await prisma.review.createMany({
    data: [
      { productId: products[0].id, userId: users[0].id, rating: 5, comment: "Fantastic laptop, super fast!" },
      { productId: products[0].id, userId: users[1].id, rating: 4, comment: "Great value, minor battery issue." },
      { productId: products[4].id, userId: users[2].id, rating: 5, comment: "Most comfortable sofa I've ever owned." },
      { productId: products[6].id, userId: users[3].id, rating: 4, comment: "Good chair, took a while to assemble." },
      { productId: products[8].id, userId: users[4].id, rating: 5, comment: "A truly captivating read." },
    ],
  });
  ...
  await prisma.$transaction(async () => {
    ...
    await prisma.payment.create({
      data: { orderId: order1.id, amount: order1.totalAmount, provider: 'Stripe', status: PaymentStatus.SUCCESS, transactionId: 'txn_alice_1' }
    });
    ...
    const order2 = await prisma.order.create({
      data: {
        userId: users[1].id,
        totalAmount: 2499.00, // Modular Cloud Sofa
        status: OrderStatus.SHIPPED,
        paymentStatus: PaymentStatus.SUCCESS,
        items: {
    ...
    await prisma.payment.create({
      data: { orderId: order2.id, amount: order2.totalAmount, provider: 'PayPal', status: PaymentStatus.SUCCESS, transactionId: 'txn_bob_1' }
    });
    ...
    await prisma.order.create({
      data: {
        userId: users[2].id,
        totalAmount: 799.50, // Photon Smartphone Pro
        status: OrderStatus.PENDING,
        paymentStatus: PaymentStatus.PENDING,
